<script>
  ...
  const MULTIPLIER_LEGEND = [
    { label: 'Single', tone: 'segment-16-20 mult-single' },
    { label: 'Double', tone: 'segment-16-20 mult-double' },
    { label: 'Triple', tone: 'segment-16-20 mult-triple' },
  ];
...
<div class="match-shell">
  <header class="topbar">
    <div class="brand">
      ...
      <div>
        <div class="brand-title">DartsLeague Live</div>
        ...
        <div class="visit-strip">
          <article class="visit-card current">
            ...
            <div class="visit-meta">
              <span>Darts left: <strong>{turnState?.dartsLeft ?? 3}</strong></span>
              {#if turnState?.checkoutHint && !turnState?.busted}
                <span>Checkout: <strong>{checkoutLabel(turnState.checkoutHint)}</strong></span>
              {:else}
                <span>Status: <strong>{turnState?.busted ? 'BUST' : 'În joc'}</strong></span>
              {/if}
            </div>
          </article>
