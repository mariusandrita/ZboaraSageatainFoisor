 ...
 * Priority: matematician > lunetist > zugrav > centrul > breakfast > hamster
 */
function detectFunnyCelebration(segment, multiplier, scoreValue, isBust, dartInTurn, prevTurnDarts, turnStartRemaining, isFinished) {
  ...
  if (dartInTurn === 3 && !isBust) {
    ...
    // Hamster: all darts hit the board but total is embarrassingly small
    if (total > 0 && total < 10 && fullTurn.every((d) => d.segment > 0)) return 'hamster';
  }
...
export default async function matchRoutes(fastify) {
  ...
  function getMatchFull(matchId) {
    ...
    return { ...match, players, legs, activeLeg, turnState };
  }
  ...
  fastify.post('/', {
    schema: {
      body: {
        ...
        properties: {
          ...
          player_ids:     { type: 'array', items: { type: 'integer' }, minItems: 1, maxItems: 5 },
        },
      },
    },
  }, async (req, reply) => {
    const { starting_score, double_out = 1, legs_to_win = 1, player_ids } = req.body;
    ...
    const create = db.transaction(() => {
      ...
      for (let i = 0; i < player_ids.length; i++) {
        db.prepare('INSERT INTO match_players (match_id, player_id, position) VALUES (?, ?, ?)').run(matchId, player_ids[i], i);
      }
  ...
  fastify.post('/:id/start', {
    ...
    db.transaction(() => {
      db.prepare("UPDATE matches SET status = 'live', started_at = datetime('now') WHERE id = ?").run(id);
  ...
  fastify.post('/:id/darts', {
    ...
    const opts = { startingScore: match.starting_score, doubleOut: match.double_out === 1, players };
    ...
    db.transaction(() => {
      ...
      if (newState.leg.finished) {
        ...
        if (mp.legs_won >= match.legs_to_win) {
          db.prepare("UPDATE matches SET status = 'finished', ended_at = datetime('now'), winner_id = ? WHERE id = ?").run(winnerId, id);
        } else {
          // Start next leg
    ...
    const dartRecord = db.prepare('SELECT * FROM darts WHERE leg_id = ? ORDER BY id DESC LIMIT 1').get(leg.id);

    // Emit WS events
    const room = matchRoom(id);
    live()?.to(room).emit(S_DART_ADDED, { dart: dartRecord, turnState });

    const funnyCelebration = detectFunnyCelebration(
      segment, multiplier, scoreValue, isBust, dartInTurn,
      prevTurnDarts, turnStartRemaining, newState.leg.finished
    );
  ...
  fastify.post('/:id/undo', {
    ...
    db.transaction(() => {
      ...
      if (!activeLeg) {
        ...
        db.prepare("UPDATE matches SET status = 'live', ended_at = NULL, winner_id = NULL WHERE id = ?").run(id);
