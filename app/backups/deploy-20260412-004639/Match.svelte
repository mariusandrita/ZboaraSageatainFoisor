<script>
  ...
  $: headerMeta = matchState
    ...
    : '';
  $: nextPlayer = activeIndex >= 0 && playerCount > 1
    ? players[(activeIndex + 1) % playerCount]
    : null;
  ...
  function playerVisitFor(playerId) {
    ...
    if (!visit) return [];
    if (playerId === currentPlayerId) return visit.current ?? [];
    return visit.lastCompleted ?? [];
  }
