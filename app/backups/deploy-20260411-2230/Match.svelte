      ...
      <div class="scoreboard-list">
        {#each playersOrdered as player, idx (player.id)}
          ...
          <article class="player-card" class:active={isActive} class:compact={compactSidebar} style="--c:{player.color}">
            ...
            {#if liveBadge}
              <div class="live-badge-row under-name">
                <BadgeToken
                  kind={liveBadge.kind}
...
<style>
  ...
  .player-card.compact .live-badge-row {
    margin-top: 0.28rem;
  }

  .player-card.compact .live-badge-row.under-name {
    padding-top: 0.35rem;
  }

  .player-card.compact .live-badge-row :global(.badge-art-shell) {
    width: 1.35rem;
    height: 1.35rem;
  }

  .player-card.compact .live-badge-row :global(.badge-token) {
    gap: 0.3rem;
  }

  .player-card.compact .live-badge-row :global(.badge-label) {
    font-size: 0.62rem;
  }

  .player-card.compact .live-badge-row :global(.badge-count) {
    font-size: 0.62rem;
  }
