<script>
  ...
  $: currentLiveBadge = currentPlayerId ? liveBadgeFor(currentPlayerId) : null;
            ...
            {#if currentLiveBadge}
              <div class="current-badge-panel">
                <div class="current-badge-copy">
                  <span class="current-badge-label">Badge live</span>
                  <strong>{currentPlayer?.name ?? 'Jucător'}</strong>
                </div>
                <div class="current-badge-token">
                  <BadgeToken
                    kind={currentLiveBadge.kind}
                    count={currentLiveBadge.count}
                  />
                </div>
              </div>
            {/if}
...
<style>
  ...
  .current-badge-token :global(.badge-token) {
    gap: 0.7rem;
  }

  .current-badge-token :global(.badge-art-shell) {
    width: 3rem;
    height: 3rem;
  }

  .current-badge-token :global(.badge-label) {
    font-size: 0.92rem;
  }

  .current-badge-token :global(.badge-count) {
    font-size: 0.84rem;
  }
